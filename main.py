...
while (mode == 1):
    print("Включить безопасный режим?\n1 - да\n2 - нет")
    k = input()

    while ((k.isdigit() == False) or (int(k) > 2) or (int(k) < 1) or (len(k) != 1)):
    ...
    if (k == 1):
        ...
        print("Введите количество переменных(максимум 27)")
        kol = input()  # количество переменных
    ...
    if (k == 1):
        while ((kol.isdigit() == False) or (len(kol) > 1) or (int(kol) > 6) or (int(kol) == 0)):  # Проверка на ввод цифр
            print("Ваш ввод должен содержать цифры меньше 7 и больше 0!\nПопробуйте ещё раз!")
            kol = input()
            
    else:
        while ((kol.isdigit() == False) or (int(kol) == 0) or (int(kol) > 64)):  # Проверка на ввод цифр
            print("Ваш ввод должен содержать любые числа кроме нуля!\nПопробуйте ещё раз!")
            kol = input()
    ...
    while (m != l):
        ...
        for i in range(l):
            if (len(znach) != l):
                print("Вы можете ввести только", l, "цифр!\nПопробуйте ещё раз!")  # правильности ввода
                i = 0
                znach = input()
                pr = list(znach)
                m -= 1
            if ((znach.isdigit() == False) or (int(pr[i]) != 1) and (int(pr[i]) != 0)):  # проверка
        ...
        if (m != 0):
            ...
            for row in tabl:
                for elem in row:
                    k += 1
                    if (k % kol == 0):
                        print(elem, znach[m], sep = "   ", end = '   ')  # Выводим элементы таблицы истинности, и если это конец строки, то выводим и значение функции при данном наборе
                    else:
                        print(elem, end = '   ')
                print()
                m += 1
            ...
            for i in range(l):
                tr[0][i] = int(znach[
                                   i])  # Подготавливаем будущий треугольник, приравнивая значения функции к первой строке треугольника
            ...
            for row in tr:
                ...
                k += 1
                print(' ' * k, end='',
                      flush=True)  # Количество пробелов зависит от строки треугольника, возрастает по мере приближения к вершине треугольника

            for i in range(l):
                zh[i] = tr[i][
                    0]  # Приравниваем левую сторону треугольника к вспомогательному масссиву, для поиска полинома Жегалкина
            ...
            for row in tabl:
                for elem in row:
                    k += 1
                    if (k % kol == 0):
                        print(elem, zh[m], sep = "   ", end = '   ')  # Выводим элементы таблицы истинности, и если это конец строки, то выводим и значение функции при данном наборе
                    else:
                        print(elem, end='   ')
                print()
                m += 1
            ...
            for i in range(l):
                if (zh[
                    i] == 1):  # Узнаём сколько единичек получилось в левой стороне, это нужно для того, чтобы знать, когда кончается полином
                    k += 1
            ...
            for i in range(l):
                ...
                if (zh[i] == 1):
                    m += 1
                if (
                        m != k):  # Если это последнее "слагаемое" в полиноме, то нам не нужно ставить после него знак XOR, поэтому мы и считаем количество единиц сверху
                    if (zh[i] == 1):
                        print(" XOR ", end='')
        else:
