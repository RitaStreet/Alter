    ...
    if (m != l):
        if (m != 0):
            ...
            for i in range(kol):
                for j in range(l):
                    tabl[j][kol - i - 1] = ((k // (
                                2 ** i)) % 2)  # Заполняем двумерный массив стандартной таблицей истинности, значения которой вычисляются по данной формуле
                    k += 1
            ...
            for row in tabl:
                for elem in row:
                    ...
                    if (k % kol == 0):
                        print(elem, znach[m],
                              end=' ')  # Выводим элементы таблицы истинности, и если это конец строки, то выводим и значение функции при данном наборе
                    else:
                        print(elem, end=' ')
                print()
                m += 1
            ...
            for i in range(l - 1):
                for j in range(l - i - 1):
                    tr[i + 1][j] = (tr[i][j] + tr[i][
                        j + 1]) % 2  # Складываем 2 верхних элемента по модулю 2, чтобы получить элемент следующей строки треугольника
